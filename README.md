...
| **Switch fabric** | QCA8719 (48-port GbE) | 🟢 Likely confirmed | `product_id` strings `"UM8719A"` / `"UM8719B"` in GPL source (`music_flash_oem.c`), matches detection list in `DGS12XX` binary |
...
**Memory-mapped switch core** (`driver/flashoem/music_flash_oem.c`) — the QCA8719 switch fabric registers are also directly memory-mapped, not solely accessible via MDIO:
...
**Existing upstream Linux support for the QCA8xxx family** — while QCA8719 itself has no known open source driver, the Linux kernel does have a mainline DSA driver (`drivers/net/dsa/qca8k.c`) for the closely related `qca8327`, `qca8334`, and `qca8337` switch chips. Given they share the same vendor and switch fabric generation, this driver is a plausible starting point for reverse-engineering or adapting a QCA8719 driver, rather than building one from scratch.
...
- **Is "Music" a real, physically distinct MIPS core, or is it integrated into the same silicon as the QCA8719 switch fabric?** The person doing this research observed that the CPU package looks similar to the adjacent switch chip on the PCB. Combined with the fact that switch-core registers are memory-mapped at `0x18800000` (8 MB) rather than solely accessed via MDIO, it's plausible the "Music" CPU core is embedded inside the same package as (or directly bonded to) the switch fabric — similar to how Realtek's RTL838x integrates a MIPS core with its switch fabric. This is unconfirmed; decapping or an Alpha Networks datasheet would settle it.
- **Why does the PRId table match AMD Alchemy Au1210?** Most likely explanation: the "Music" core was designed to expose a MIPS 4Kc-compatible PRId for toolchain/kernel compatibility, coincidentally or deliberately matching an existing PRId value rather than registering a new one. This is common when reusing an existing MIPS core license or wanting off-the-shelf GCC/kernel support without upstream changes.
- **Does Alpha Networks Incorporation have public documentation for the "Music" platform?** They are the ODM that physically designed this board (confirmed via GPL source copyright headers). No public datasheet has been located yet.
...
| QCA8719 switch fabric driver | ❌ None (QCA8719-specific) | 🟡 But mainline `drivers/net/dsa/qca8k.c` supports the related qca8327/qca8334/qca8337 family — plausible adaptation base |
...
2. **Adapt or reverse-engineer a QCA8719 DSA driver** — start from mainline `qca8k.c` (qca8327/8334/8337 family) and cross-reference against the GPL `music_mdiom_switch_read/write` calls and the memory-mapped switch-core register space to identify QCA8719-specific differences. The switch-core register base is confirmed live (`0x18800000` reads `0x02022002`, decoding to revision "Vivo" B0).
...
- Full OpenWrt with all 48 ports: still likely 4-8 months — shorter than originally estimated since the MDIO protocol is now fully documented from source *and* proven working on real hardware, but the QCA8719 switch driver and CPU platform port to a modern kernel both still need to be written.
...
- **Reverse engineering:** Cross-referencing the GPL kernel MDIO source against a QCA8xxx family datasheet (qca8327/8334/8337 public documentation) to identify QCA8719-specific register differences
...
| **Switch fabric** | QCA8719 (48 ports GbE) | 🟢 Probablement confirmé | Chaînes `"UM8719A"` / `"UM8719B"` dans le code source GPL (`music_flash_oem.c`), cohérent avec la liste de détection du binaire `DGS12XX` |
...
- OpenWrt complet avec les 48 ports : 4-8 mois — plus court qu'estimé initialement, mais le driver switch QCA8719 et le portage de la plateforme CPU restent à écrire
...
- **Reverse engineering :** Croiser le code source MDIO du kernel GPL avec une datasheet de la famille QCA8xxx (documentation publique qca8327/8334/8337) pour identifier les différences spécifiques au QCA8719
